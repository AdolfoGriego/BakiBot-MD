...
if (/video/g.test(mime)) if ((q.msg || q).seconds > 8) return await this.sendButton(m.chat, '*EL VIDEO NO PUEDE DURAR MAS DE 7 SEGUNDO*', wm, [['DESACTIVAR AUTOSTICKER', '/disable autosticker']], m)
let img = await q.download()
if (!img) return
...
if (stiker) {
let img = await(await fetch('https://github.com/AdolfoGriego.png')).buffer()  
