...
global.authFile = `BakiBotSessions`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['BakiBot-MD', 'Edge', '20.0.04'] : methodCodeQR ? ['BakiBot-MD', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./BakiBotSessions")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./BakiBotSessions/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./BakiJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./BakiJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./BakiJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./BakiJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./BakiBotSessions/', './BakiJadiBot/']
